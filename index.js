...
app.post('/questions', async (req, res) => {
  ...
  res
    .status(newQuestion ? 201 : 400)
    .json(newQuestion ?? {error: 'We\'re sorry but the question could not be added.'})
})
...
app.post('/questions/:questionId/answers', async (req, res) => {
  const newAnswer = await req.repositories.questionRepo.addAnswer(req.params.questionId, req.body)

  res
    .status(newAnswer ? 201 : 400)
    .json(newAnswer ?? {error: 'We\'re sorry but the answer could not be added.'})
})
